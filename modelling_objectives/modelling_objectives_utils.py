...
def plot_interpolation(model, device, latent_dim_size):
...
def test_conv_transpose1d(conv_transpose1d, n_tests=10, verbose=False):
    import numpy as np
...
def test_fractional_stride_1d(fractional_stride_1d, verbose=False):
...
def test_conv_transpose2d(conv_transpose2d, n_tests=5, verbose=False):
    import numpy as np
    for i in range(n_tests):
        b = np.random.randint(1, 8)
        h = np.random.randint(10, 50)
